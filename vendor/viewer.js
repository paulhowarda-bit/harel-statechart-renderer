/* viewer.js — renders the laid-out Harel statechart and drives all interaction.
   Expects globals: GRAPH (laid-out graph), d3, and (for paragraph collapse/expand)
   window.__relayout / __grouping / __collapsed set up by layout_boot.js.
   The drawing lives in render(), which is re-callable so collapse/expand can
   re-lay-out and redraw without re-wiring the one-time controls/zoom/search. */
(function () {
  "use strict";
  let G = window.GRAPH;
  ...
  // Layer order = paint order. Group outlines sit at the very back, then nodes,
  // then the boundary, then edges, then annotations on top.
  const gGroups = root.append("g").attr("class", "para-groups");
  ...
  // ---- pure helpers (closures over the current G/nodeById) --------------------
  ...
  function render() {
    ...
    nodeSel.append("rect").attr("class", "box")
      .attr("width", d => d.width).attr("height", d => d.height).attr("rx", 8).attr("ry", 8);
    ...
    // "N steps ▸" badge on a collapsed paragraph — signals it's expandable
    nodeSel.filter(d => d.isCollapsedGroup).append("text").attr("class", "group-count")
      .attr("x", d => d.width - 8).attr("y", d => d.height - 7).attr("text-anchor", "end")
      .text(d => `${d.memberCount} steps ▸`);
    ...
    edgeSel.each(function (e) {
      ...
      if (!label.cap && !label.ac) return;
      let lx, ly, anchor;
      if (e.labelPos) { lx = e.labelPos.x; ly = e.labelPos.y + 10; anchor = "start"; }
      else {
      ...
      if (near && near.dist > 10) {
        g.append("line").attr("class", "leader lod-l2")
          .attr("x1", lx).attr("y1", ly - 4).attr("x2", near.pt.x).attr("y2", near.pt.y);
    ...
    const bSel = gBoundary.selectAll("g.boundary").data(G.boundaryNodes || [], d => d.id)
      .enter().append("g")
    ...
    bSel.append("text").attr("class", "ep-kind").attr("x", 8).attr("y", 14).text(d => d.kind);
    ...
    const beSel = gBoundaryEdges.selectAll("g.bedge").data(G.boundaryEdges || [], d => d.id)
      .enter().append("g")
    ...
    beSel.append("path").attr("class", "hit").attr("d", bedgePath)
      .style("stroke", "transparent").style("stroke-width", "24px")
      .style("fill", "none").style("pointer-events", "stroke");
    beSel.append("title").text(d => `${d.direction === "in" ? "input" : "output"} · ${d.label}`);

    // per-state in/out badges
    gNodes.selectAll("g.state").each(function (d) {
      ...
      const mk = (items, side) => {
        ...
        items.forEach(it => {
          const bw = 7;
          const grp = g.append("g").attr("class", `io-badge ${side}${it.unconfirmed ? " unconfirmed" : ""}`);
          grp.append("circle").attr("cx", side === "in" ? -bw : d.width + bw).attr("cy", yy + 6).attr("r", 4);
          grp.append("title").text((it.kind === "event" ? "event " : "field ") + it.label
    ...
    // mini-map nodes
    miniNodes.selectAll("rect.mini-node").remove();
    miniNodes.selectAll("rect.mini-node")
      .data(G.nodes.filter(n => !n.isContainer || n.kind === "and")).enter().append("rect")
      .attr("class", d => "mini-node" + (d.kind === "and" ? " and" : ""))
      .attr("x", d => d.x).attr("y", d => d.y).attr("width", d => d.width).attr("height", d => d.height);

    if (searchBox && searchBox.value) runSearch();
  }

  // ---- paragraph collapse / expand -------------------------------------------
  let collapsed = new Set(window.__collapsed || []);
  const canGroup = () => G.grouping && G.grouping.enabled && typeof window.__relayout === "function";

  function relayoutAndRender(keepView) {
    if (!canGroup()) return;
    const t = d3.zoomTransform(svg.node());
    setBoot("Re-laying out " + collapsed.size + " collapsed paragraph(s)…");
    window.__relayout([...collapsed]).then(g => {
      G = g; window.GRAPH = g; window.__collapsed = [...collapsed];
      setBoot(null);
      render();
      mini.attr("viewBox", `0 0 ${G.width} ${G.height}`);
      if (keepView) { svg.call(zoom.transform, t); updateMinimapViewport(t); }
      else { initialView(); }
    }).catch(err => { setBoot("Layout failed: " + (err && err.message || err), true); });
  }
  ...
  function collapseAll() { if (!canGroup()) return; collapsed = new Set(G.grouping.order); relayoutAndRender(false); }
  function expandAll() { if (!canGroup()) return; collapsed = new Set(); relayoutAndRender(false); }

  function setBoot(msg, isError) {
    const el = document.getElementById("boot-status");
    if (!el) return;
    if (msg === null) { el.style.display = "none"; return; }
    el.style.display = "flex"; el.className = isError ? "error" : "";
    const m = el.querySelector(".boot-msg"); if (m) m.textContent = msg;
  }
  ...
  const miniNodes = mini.append("g").attr("class", "mini-nodes");
  ...
  function runSearch() {
    ...
    gBoundaryEdges.selectAll("g.bedge").classed("match", d => epHits.has(d.endpoint) || stateHits.has(d.state));
    matches = [...stateHits].filter(id => nodeById[id]);
  ...
  function applyFilter(stateHits, edgeHits, radius) {
    ...
    gEdges.selectAll("g.edge").classed("dim", d => !(keep.has(d.source) && keep.has(d.target)) && !edgeHits.has(d.id));
  }
  ...
  function inspectState(d) {
    ...
    html += `<h3>${d.label}</h3><span class="kind">${d.isCollapsedGroup ? "paragraph (" + d.memberCount + " states — click to expand)" : kindLabel(d.kind)}</span>`;
  ...
  function inspectEdge(e) {
    ...
    html += `<div class="row"><span class="k">label:</span> ${lab.cap || ""} ${lab.ac || ""}</div>`;
    el.innerHTML = html; el.classList.add("show");
  }
  ...
  function nodeTooltipHTML(d) {
    ...
    s += `<div class="tt-kind">${d.isCollapsedGroup ? "paragraph · " + d.memberCount + " states" : esc(kindLabel(d.kind))}` +
      (d.kind === "history" && d.historyDepth ? ` · ${esc(d.historyDepth)}` : "") + `</div>`;
    if (d.description) s += `<div class="tt-row tt-desc">${esc(d.description)}</div>`;
    if (d.entry && d.entry.length) s += ttRow("on enter", d.entry.map(esc).join("; "), "tt-entry");
    if (d.exit && d.exit.length) s += ttRow("on exit", d.exit.map(esc).join("; "), "tt-exit");
    if (h.staticReactions && h.staticReactions.length) s += ttRow("reactions", h.staticReactions.map(esc).join("; "), "tt-sr");
    if (h.activities && h.activities.length) s += ttRow("do (activity)", h.activities.map(a => `${esc(a.name)} (${esc(a.binding)})`).join("; "), "tt-do");
    if (h.broadcast && h.broadcast.length) s += ttRow("broadcast", h.broadcast.map(b => `${esc(b.event)}: ${esc(b.from)}→${(b.to || []).map(esc).join(",")}`).join("; ") + (h.sensing ? ` (${esc(h.sensing)})` : ""), "tt-bc");
    if (d.provenance && d.provenance.cobolParagraph) {
      const p = d.provenance;
      s += ttRow("COBOL", `${esc(p.file || "")} ${esc(p.cobolParagraph)}` + (p.sourceLines ? ` (lines ${p.sourceLines.join("–")})` : ""), "tt-src");
    ...
    const b = d.ioBadges;
    if (b && b.in && b.in.length) s += ttRow("inputs", b.in.map(it => `${it.kind === "event" ? "event" : "field"} ${esc(it.label)}` + (it.endpoint ? ` ← ${esc(epLabel(it.endpoint))}` : (it.unconfirmed ? " (endpoint unconfirmed)" : ""))).join("; "), "tt-in");
    if (b && b.out && b.out.length) s += ttRow("outputs", b.out.map(it => `${it.kind === "event" ? "event" : "field"} ${esc(it.label)}` + (it.endpoint ? ` → ${esc(epLabel(it.endpoint))}` : "")).join("; "), "tt-out");
    s += `<div class="tt-hint">${d.isCollapsedGroup ? "click to expand this paragraph" : "click to pin details"}</div>`;
    return s;
  }
  ...
  function boundaryNodeTooltipHTML(d) {
    ...
    const ins = (G.index.inputs || []).filter(io => io.endpoint === d.endpointId);
    const outs = (G.index.outputs || []).filter(io => io.endpoint === d.endpointId);
    if (ins.length) s += ttRow("into program", ins.map(io => `${esc(io.label)} → ${stLabel(io.state)}`).join("; "), "tt-in");
    if (outs.length) s += ttRow("out of program", outs.map(io => `${stLabel(io.state)} → ${esc(io.label)}`).join("; "), "tt-out");
  ...
  // paragraph collapse/expand buttons (only for grouped COBOL machines)
  if (canGroup()) {
    const bar = document.getElementById("bar");
    const spacer = bar.querySelector(".spacer");
    const mk = (label, title, fn) => {
      const b = document.createElement("button");
      b.textContent = label; b.title = title; b.className = "para-btn";
      b.addEventListener("click", fn);
      bar.insertBefore(b, spacer ? spacer.nextSibling : null);
      return b;
    };
    mk("⊟ Collapse all", "Fold every paragraph to one node (overview)", collapseAll);
    mk("⊞ Expand all", "Show every paragraph's internal control flow", expandAll);
  }
  ...
  document.addEventListener("keydown", (ev) => {
    ...
    else if (ev.key === "c" && canGroup()) collapseAll();
    else if (ev.key === "x" && canGroup()) expandAll();
